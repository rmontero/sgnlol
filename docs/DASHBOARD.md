...
Admins can use **Deliveries → Send a tracked test alert** for an existing configured destination. The app commits the test record before calling Slack, then records `sent` with Slack's timestamp, `failed` for an explicit rejection, or `unknown` when the result is uncertain. Tests do not create scored events or consume AI credits. Repeating the same request ID returns the existing outcome without another Slack send. Use **Start another test** only when deliberately sending a new message.

The report covers app-tracked deliveries, not every message in Slack. Avoid direct `chat.postMessage` calls for tests because they bypass this outbox. API clients use `POST /api/dashboard/test-alert` with `org_id`, `recipient`, and a unique UUID `request_id`, plus admin authentication and the request-verification header. Previously confirmed external tests can be recorded by an admin through `/api/dashboard/test-alert/receipt` with `org_id`, `recipient`, `channel`, and `slack_ts`. Such rows are labeled imported Slack confirmations, retain the original Slack time, are idempotent, and never resend a message. Only import a receipt backed by an actual successful Slack response.

## Navigation and reporting

Dashboard screens have shareable links such as `/dashboard#sources` and `/dashboard#deliveries`. On mobile, **Menu** exposes every screen. All-time analytics are collapsed on the inbox and hidden on task screens. Historical fallback counts describe past events, not current scorer health.

**Deliveries** supports status, type, and date-window filters. Test alerts and imported confirmations are labeled separately from scored alerts. Expand a row for the original error, provider receipt, and next-step guidance. The test tool is collapsed by default and uses configured organization/destination selectors; sending another test requires an explicit new-test action.

**Sources** shows the last accepted event and stored-event count. “Awaiting first accepted event” does not claim a disconnected integration, and receiving an event previously does not prove current provider health. Add/edit opens a dialog with optional display names, grouped settings, and persistent Save controls. Close keeps an unsaved draft; Add source reopens it. Discard draft explicitly clears it. Drafts survive screen navigation but are not saved across page reloads; the browser warns before leaving with unsaved source/account edits.
